...
void setup()
{
	...
	TCCR1B   =   0x1A;
#if HeadTracker == 1
    Wire.begin(UltimateLRS_ADDR);                // join i2c bus with address #0x11
...
//  digitalWrite(BUZZER, HIGH);
   ...
   {
     headtrackerchannel[i] = Wire.read(); // receive byte as a character
     if(i<5)
      i++ ;
   }
...
void loop()
{
	...
	while (1)
	{
		...
		if (time > old_time + 30)
		{
          ...
          if(datareceived)
          {
            Servo_Buffer[5 + 1] = (headtrackerchannel[0] << 8) + headtrackerchannel[1] ;
            Servo_Buffer[6 + 1] = (headtrackerchannel[2] << 8) + headtrackerchannel[3] ;
            datareceived = 0 ;
          if (debug)
          {
            Serial.print("s");
           // Serial.print(i);
            Serial.print(headtrackerchannel[0],HEX);
            Serial.print(headtrackerchannel[1],HEX);
            Serial.print(" - ") ; 
            Serial.print(headtrackerchannel[2],HEX);
            Serial.print(headtrackerchannel[3],HEX);
            Serial.print(" - ") ; 
            Serial.print(headtrackerchannel[4],HEX);
            Serial.print(headtrackerchannel[5],HEX);
            Serial.print(" - ") ; 
            Serial.print(Servo_Buffer[5 + 1],HEX);
            Serial.print(" - ") ; 
            Serial.print(Servo_Buffer[6 + 1],HEX);
            Serial.println();
