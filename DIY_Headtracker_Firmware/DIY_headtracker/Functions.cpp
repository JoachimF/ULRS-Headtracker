...
void Send_i2C()
{
  ...
  chan1 = channel_value[htChannels[0]]+900 ;
  chan2 = channel_value[htChannels[1]]+900 ;
  chan3 = channel_value[htChannels[2]]+900 ;
   Wire.beginTransmission(UltimateLRS_ADDR); // transmit to device #11
   Wire.write(chan1 >> 8);        // MSB
   Wire.write(chan1 & 0xFF);        // LSB
   Wire.write(chan2 >> 8);        // MSB
   Wire.write(chan2 & 0xFF);        // LSB
   Wire.write(chan3 >> 8);        // MSB
   Wire.write(chan3 & 0xFF);        // LSB
   Wire.endTransmission();    // stop transmitting
/*   Serial.print(chan1 >> 8,HEX);
   Serial.print(chan1 & 0xFF,HEX);
   Serial.print(" - ") ; 
   Serial.print(chan2 >> 8,HEX);
   Serial.print(chan2 & 0xFF,HEX);
   Serial.print(" - ") ; 
   Serial.print(chan2 >> 8,HEX);
   Serial.print(chan2 & 0xFF,HEX);
   Serial.println();*/
